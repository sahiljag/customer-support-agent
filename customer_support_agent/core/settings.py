...
class Settings(BaseSettings):
    ...
    groq_api_key:str = ""
    groq_model:str = "llma-3.1-8b-instant"
    llma_temperature:float = 0.2
    ...
    def ensure_directories(settings:Settings | None = None) -> None:
        """ create local directories required by sqlite nad chromadb"""
        config = settings or Settings.get_settings()

        for path in (
            config.reslove(config.data_dir),
